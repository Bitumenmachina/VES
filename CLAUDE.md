...
- build: 2.1.0-rc.3 (Batch Q3, VES 2.1 — SELECTION AFFORDANCES: the picked thing announces itself and offers its doors. A selection chip beside the pointer (trade word · condition · value · sheet · N more under it) with Edit · Delete · Move · Duplicate · Re-assign · Cycle; `state.selectedIds` beside `selectedId` (grips only when one is selected); Shift+click adds, a drag on EMPTY paper sweeps a marquee that skips hidden conditions, Esc clears; pointerdown on the BODY of an already-selected shape moves it (grips win inside 8 px; an unselected body still pans; a press that never travelled 3 px is a click, so a repeat click still steps the stack — AF-1 stands); Ctrl+D duplicates at +12/+12 and lands selected; Shift+R files the selection under another condition of the SAME kind (plain R still rotates; the wrong kind is refused with the reason spoken); bulk Delete / Move / Re-assign are ONE journal entry, one Ctrl+Z; Shift+Space steps the stack under the pointer; a hidden region keeps a faint ghost chip carrying its eye, and Show all clears hiddenRegions too (Q3-0, R-12a). Screen-side only: no money rule, no file version, no print path moved.)
- bytes: 3657712
