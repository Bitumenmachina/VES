...
| T-01 | P-TRADE | The "client review" .xlsx (recap Summary tab door, file `*-client-review.xlsx`) exports COST — Recap tab TOTAL 51,804.12 while the bid for the same state prints Total $64,620.46; two client-labelled surfaces $12,816.34 apart; `clientReviewModel` buckets `l.extended` with no ladder (confirmed in bytes) | **(a) money-honesty, client paper carries internal cost (D-24.2 class) — fix this wave: proposed Batch AB-1, the review sheet carries SELL (ladder factor per line, cents apportioned so the sheet sums to the bid Total); red-first probe: XLSX total == bid total, no cost column on a client-named file** |
| T-02 | P-TRADE | A saved takeoff re-prices on another machine with no signal: the JSON carries conditions, measurements and overrides but no library and no library fingerprint; loaded into a fresh profile it silently priced $64,620.46 where it was saved at $73,352.26 (edited library) — toast "Takeoff loaded." | **(a) two surfaces disagree, silently — proposed Batch AB-2: the file carries the library's name + fingerprint; the load door compares and raises the standing banner when they differ ("priced with library X here, Y in the file"). Embedding the whole library in the takeoff is a design CANDIDATE for Patrick (it changes what the file is).** |
| T-03 | P-TRADE | Estimate CSV footer carries Cost and Sell but no Overhead / Markup / Profit rows — the reader cannot see how the sell was reached; the ladder exists as values only on the cost-sheet PDF | (b) bar row M2 class (exports that lose the math) — proposed Batch AB-3: three ladder rows in the CSV footer, apportioned cents, sum == Sell |
| T-04 | P-TRADE | Every XLSX carries values only, no `<f>` formulas (client review 2 sheets, library 5 sheets) | (b) bar row M2 — queued as B7 (not yet approved) |
| T-05 | P-TRADE | Files & exports menu offers no .xlsx; "Review .xlsx" lives on the recap Summary tab, "Export library (.xlsx)" sits at y=947 in the Setup modal (scrolls) | (c) CANDIDATE — list the review door in the exports menu |
| T-06 | P-TRADE | Toast text stays stale across a print (RFQ export text still showing after the cost sheet printed) | (c) CANDIDATE — the print path could clear or set the toast |
