...
- build: 2.1.0-rc.4 (Batch Q2F, VES 2.1 — the one fix batch after Q2's regression was caught by the gate on the runner (CI on ebbea2c, probes job red at probe-z Z5): at phone width the file:// notice banner overflowed its 63px box across the rail's cards and, with the open rail at z-index auto, took the long-press that opens the money peek; Q2's strip had moved the first card down into that band. The open rail in the narrow layout now stacks above the stage's banners (z-index 21; R-14b). One CSS rule; no money, no file, no print path.)
- bytes: 3657712
