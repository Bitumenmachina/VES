...
| P3-03 | Clicking a condition leaves the old one armed; four states on one card (selected / armed / editing / expanded) | (a) **FIXED F18.69 (Batch AG)** — P3-R1 applied: a click arms, the armed card stays armed, ⌖/Esc/pill stop, the detail follows the arm, the highlight-only state removed; probe-ag 5/5 (RED-first 0/5 on F18.68) |
| P3-04 | Pitch reads as a multiplier ("1.118×"); should read as rise over 12 ("6/12") | (a) **FIXED F18.70 (Batch AH)** — faces only (chip, both fields, toasts, rollup CSV + 'Pitch factor' column, takeoff page); storage stays the multiplier, G0 4/4; probe-ah 6/6 (RED-first 0/6 on F18.69) |
| P3-05 | The exports menu (15 flat items, "one breath" captions) and the left rack read as prototype; captions are AI-speak, not trade words | (a) **FIXED F18.71 (Batch AI)** — P3-R2 + P3-R3 applied: four plain menu sections, working CSVs under Setup, no Type group on the rack, Setup headings and the price-source legend in trade words; probe-ai 5/5 (RED-first 0/5 on F18.70) |
...
| AH-2 | seat (probe) | On F18.69 a bare "6" typed into a pitch field stored a **6× multiplier** (six times the quantities and money): the old grammar read anything without "/12" as a multiplier — probe-ah AH2 RED | (a) FIXED F18.70 — a bare rise up to 24 means rise/12; only a decimal ≥ 1 is a multiplier |
