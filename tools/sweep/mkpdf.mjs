// Synthetic 36x24in "plan" PDF: vector paths only, no fonts, no client data. Args: <out.pdf> [segments=1500] [pages=1]
// (Batch AE: a page count, so a probe can print a takeoff across more than one sheet).
...
const PAGES = Math.max(1, +(process.argv[4] || 1));
const objs = [];
const kids = []; for (let k = 0; k < PAGES; k++) kids.push(`${3 + k * 2} 0 R`);
objs.push('<< /Type /Catalog /Pages 2 0 R >>'); objs.push(`<< /Type /Pages /Kids [${kids.join(' ')}] /Count ${PAGES} >>`);
for (let k = 0; k < PAGES; k++) { objs.push(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${W} ${H}] /Contents ${4 + k * 2} 0 R /Resources << >> >>`); objs.push(`<< /Length ${Buffer.byteLength(s)} >>\nstream\n${s}endstream`); }
let out = '%PDF-1.4\n'; const offs = [];
